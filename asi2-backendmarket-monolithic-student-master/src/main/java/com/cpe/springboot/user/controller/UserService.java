...
import com.cpe.springboot.user.model.*;
...
public class UserService {
	...
	public UserDTOFlag addUser(UserDTO user) {
		UserModel u = fromUDtoToUModel(user);
		// needed to avoid detached entity passed to persist error
		UserDTOFlag userDTOFlag = new UserDTOFlag(Flag.PENDING, Action.ADD, user);
		busEmitter.sendMsg(userDTOFlag);
		return (userDTOFlag);
	}

	public  void  addUserDb(UserDTOFlag user) {
		user.setFlag(Flag.SUCCESS);
		// notifier l'utilisateur avec une socket
	...
	public UserDTOFlag updateUser(UserDTO user) {
		UserModel u = fromUDtoToUModel(user);
		// needed to avoid detached entity passed to persist error
		UserDTOFlag userDTOFlag = new UserDTOFlag(Flag.PENDING, Action.UPDATE,user);
		busEmitter.sendMsg(userDTOFlag);
		return (userDTOFlag);
	}

	public void updateUserDB(UserDTOFlag user) {
		user.setFlag(Flag.SUCCESS);
		// notifier l'utilisateur avec une socket
		UserDTO userDTO = user.getUserDTO();
		UserModel u = fromUDtoToUModel(userDTO);
